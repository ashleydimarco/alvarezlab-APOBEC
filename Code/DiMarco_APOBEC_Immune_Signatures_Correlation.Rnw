...
library(circlize)
library(MASS)
root <- "/data1/workspace/DCI/Force"
root <- "~/Desktop/Alvarez"
dir_data <- file.path(root, "Data")
# 
# load(file.path(dir_data, "BRCA_Expression.RData"))
# load(file.path(dir_data, "BRCA_MAF.RData"))
# load(file.path(root, "APOBEC/Data/BRCA_Clinical.RData")) # clinical data
# load(file.path(root, "APOBEC/Data/BRCA_MAF.RData"))  # mutation data
# load(file.path(root, "Data/cleaned.RData"))   # expression data
# load(file.path(root, "tmp.RData"))
# load(file.path(root, "Immune_Signature/Results/DiMarco-APOBEC-Immune-Signature-Data.RData"))
@ 
\end{frame}
...
if(run == F){
  load(file.path(dir_data, "tcga-brca_cleaned.RData"))
} else {
  if(preprocess == F){
    load(file.path(dir_data, "tcga-brca_apobec-enrichment_mutect2.RData"))
    load(file.path(dir_data, "tcga-brca_rse_proc.RData"))
    load(file.path(dir_data, "signatures_immune.RData"))
  } else {
    if(query == F){
      load(file.path(dir_data, "fasta_grch38_chr24.RData"))
      load(file.path(dir_data, "tcga-brca_maf_mutect2.RData"))
      load(file.path(dir_data, "tcga-brca_rse_raw.RData"))
    }
  } 
}
@
...
<<eval=query, results="markup">>=
# https://gdc.cancer.gov/about-data/data-harmonization-and-generation/gdc-reference-files
# Fasta
grch38 <- readDNAStringSet(file.path(wd, "Data/Reference/GRCh38.d1.vd1.fa"))
chr24 <- grch38[1:24]
...
save(chr24,
     file = file.path(dir_data, "fasta_grch38_chr24.RData"))
@
\end{frame}
...
<<eval=query, echo=F, results="markup">>=
# 06/15/2018
maf_mt2 <- GDCquery_Maf(tumor = "BRCA", 
                        pipelines = "mutect2",
                        save.csv = T, 
                        directory = file.path(dir_data, "TCGA-BRCA"))
save(maf_mt2,
     file = file.path(dir_data, "tcga-brca_maf_mutect2.RData"))
@

<<eval=preprocess, echo=F, results='markup'>>=
snv_mt2 <- maf_mt2 %>%
...
save(snv_mt2, es_mt2, 
     file = file.path(dir_data, "tcga-brca_apobec-enrichment_mutect2.RData"))
@
\end{frame}
...
save(query_rse, rse_raw,
     file = file.path(dir_data, "tcga-brca_rse_raw.RData"))
@

<<eval=preprocess, results="markup">>=
gdat_cnt <- assay(rse_brca)
pdat <- colData(rse_brca)
anno <- rowData(rse_brca)
...
save(rse_proc, file = file.path(dir_data, "tcga-brca_rse_proc.RData"))
@
\end{frame}


\subsection{Immune Signatures}
<<eval=preprocess, echo=F, results="asis">>=
...
save(tab_sigs,
     file = file.path(dir_data, "signatures_immune.RData"))
@
...
<<eval=run, echo=F, eval=preprocess>>=
expr_sigs <- tab_sigs %>%
...
expr_sigs_pc1 <- expr_sigs %>%
  # filter(group %in% c("B.cells","T.cells")) %>%
  group_by(group) %>%
...
save(combined_avg, combined_pc1, tab_sigs,
     file = file.path(dir_data, "tcga-brca_cleaned.RData"))
@
...
groups <- setdiff(unique(tab_sigs$group), "APOBEC3B")
@
...
for(subtype in subtypes){
  ...
  if(subtype != "All-Patients"){
    tmp1 <- tmp[tmp$PAM50 == subtype, ]
  }
  pids_low <- with(tmp1, rownames(tmp1)[APOBEC <= cutoff_apobec])
  pids_high <- with(tmp1, rownames(tmp1)[APOBEC > cutoff_apobec])

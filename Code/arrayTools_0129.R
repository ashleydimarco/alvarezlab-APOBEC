...
library(multtest)
library(pamr)
source("C:/Users/markia/OneDrive - University of North Carolina at Chapel Hill/Hoadley_Troester/R/clanc.R") #train, build, test
library(affy)
library(limma)
library(qvalue)
library(e1071)
...
affy2name.hs<- function(x){
	ensembl = useMart("ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl")
	ann<- getBM(attributes=c('affy_hg_u133_plus_2','hgnc_symbol'),filters='affy_u133_x3p',values=rownames(x),mart= ensembl)
	ann<- ann[!duplicated(ann$ensembl_gene_id),]
	rownames(ann)<- ann$ensembl_gene_id

	xe<- x[ann[,1],]
	xc<- collapseIDs(xe,as.vector(ann[,2]),method="stdev")
	xc<- xc[!is.na(xc[,1]),]
	return(xc)
}

genbank2entrez.hs<- function(x){
	...
	ann<- getBM(attributes=c('refseq_mrna','entrezgene'),filters='refseq_mrna',values=rownames(x),mart= ensembl)
	ann<- ann[!duplicated(ann$refseq_mrna),]
	rownames(ann)<- ann$refseq_mrna
...
genbank2name.hs<- function(x){
	#ensembl = useMart("ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl",host="www.ensembl.org")
	# use this if above doesn't work
	ensembl = useMart(biomart = "ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl", host = "useast.ensembl.org")
	ann<- getBM(attributes=c('refseq_mrna','hgnc_symbol'),filters='refseq_mrna',values=rownames(x),mart= ensembl)
	ann<- ann[!duplicated(ann$refseq_mrna),]
	rownames(ann)<- ann$refseq_mrna
...
ens2name.hs<- function(x){
	ensembl = useMart("ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl",host="www.ensembl.org")
	# use this if above doesn't work
	#ensembl = useMart(biomart = "ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl", host = "useast.ensembl.org")
	ann<- getBM(attributes=c('ensembl_gene_id','hgnc_symbol'),filters='ensembl_gene_id',values=rownames(x),mart= ensembl)
	ann<- ann[!duplicated(ann$ensembl_gene_id),]
	rownames(ann)<- ann$ensembl_gene_id

	xe<- x[ann[,1],]
	xc<- collapseIDs(xe,as.vector(ann[,2]),method="sum")
	xc<- xc[!is.na(xc[,1]),]
	return(xc)
}

name2entrez.hs<- function(x){
	ensembl = useMart("ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl",host="www.ensembl.org")
	# use this if above doesn't work
	#ensembl = useMart(biomart = "ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl", host = "useast.ensembl.org")
	#ensembl = useMart(biomart = "ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl", host = "jul2015.archive.ensembl.org")
	ann<- getBM(attributes=c('hgnc_symbol','entrezgene'),filters='hgnc_symbol',values=rownames(x),mart= ensembl)
	ann<- ann[!duplicated(ann$hgnc_symbol),]
	rownames(ann)<- ann$hgnc_symbol
...
entrez2name.hs<- function(x,m="stdev"){
	#ensembl = useMart("ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl",host="www.ensembl.org")
	# use this if above doesn't work
	ensembl = useMart(biomart = "ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl", host = "jul2015.archive.ensembl.org")
	ann<- getBM(attributes=c('hgnc_symbol','entrezgene'),filters='entrezgene',values=rownames(x),mart= ensembl)
	ann<- ann[!duplicated(ann$entrezgene),]
	rownames(ann)<- ann$entrezgene

	xe<- x[as.character(ann[,2]),]
	xc<- collapseIDs(xe,as.vector(ann[,1]),method=m)
	xc<- xc[!is.na(xc[,1]),]
	return(xc)
	
}

name2entrez.mm<- function(x){
	#ensembl = useMart("ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl",host="www.ensembl.org")
	# use this if above doesn't work
	...
	ann<- getBM(attributes=c('mgi_symbol','entrezgene'),filters='mgi_symbol',values=rownames(x),mart= ensembl)
	ann<- ann[!duplicated(ann$mgi_symbol),]
	rownames(ann)<- ann$mgi_symbol
...
nanoNorm<- function(x,types,hkplot=T,hkpenalty=0.5){
	####
	# bg threshold
	####
	negc<- grep("Negative",types)
	negc.mean<- apply(x[negc,],2,mean)
	negc.sd<- apply(x[negc,],2,sd)
	x<- t(apply(x,1,function(x,y){
														x[x<=y]<-y[x<=y]
														return(x)
													},negc.mean+2*negc.sd))
	...
	###
	# normalize to housekeepers
	###
	house<-grep("Housekeeping",types)
	x.house<- x[house,]
	x.house.Crank<- apply(length(house)-apply(x.house,2,rank)+1,1,mean)
	x.house.Srank<- rank(apply(x.house,1,sd))
	fit<-lm(x.house.Srank~x.house.Crank)
	filt<- abs(fit$residuals) < hkpenalty
	if(hkplot){
		plot(x.house.Crank,x.house.Srank,pch="")
		abline(fit)
		text(x.house.Crank[filt],x.house.Srank[filt],names(x.house.Crank)[filt],col="black")
		text(x.house.Crank[!filt],x.house.Srank[!filt],names(x.house.Crank)[!filt],col="red")
	
	}
	normFactors<-apply(x[house,],2,function(x){return( 2^mean(log2(x)) )})
	normFactors<- median(normFactors) / normFactors
	x<- t(apply(x,1,function(x,y){x*y},normFactors))
	
	###
	# remove control genes
	###
	rmthese<- c(grep("Housekeeping",types),grep("Negative",types),grep("Positive",types))
	x<- x[-1*rmthese,]														

	return(x)
}

path.test.perm<-function(x,y,perm=F){
                flag<- (names(y) %in% x)
                if(sum(flag) < 15){
                	return(c(NA,NA,NA,NA,sum(flag)))
                }
                flag<- flag+1
                p<- t.test(y~flag)
                wrs.p<- wilcox.test(y~flag)
                #ks.p<- ks.test(y[flag],y[!flag])
                if(perm){
	                rt<- rep(NA,100)
  	              for(i in 1:1000){
    	            	flag<- sample(flag,length(flag))
      	          	rp<- t.test(y~flag)
        	        	rt[i]<- rp$statistic
          	      }
            	    fn<- ecdf(rt)
              	  return(c(p$estimate,2*min(fn(-p$statistic),fn(p$statistic)),sum(flag==2)))
              	}else{
              		return(c(p$estimate,p$p.value,wrs.p$p.value,sum(flag==2)))
              	}
}

path.test<-function(x,y){
                flag<- (names(y) %in% x)+1
                if(sum(flag==2)>10){
	                p<- t.test(y~flag)
  	           	}else{
  	           		p<- list(estimate=c(NA,NA),p.value=NA)
  	           	}
  	            return(c(p$estimate,p$p.value))
  	             
}
pathway.test<-function(scores,gmt){
	path.tx<- lapply(gmt$genesets,path.test,scores)
	# format and annotate the output
	path.tx<- matrix(unlist(path.tx),ncol=3,byrow=T)
	rownames(path.tx)<- gmt$geneset.names         
	gsn<- sapply(gmt$genesets,function(x,y){sum(x %in% y)},names(scores))
	path.tx<- data.frame(path.tx,gsn)
	rownames(path.tx)<- gmt$geneset.names 
	path.tx<- path.tx[sort.list(path.tx[,3]),]
	colnames(path.tx)<- c("global mean","set mean","p-value","genesInSet")
	return(path.tx)
}
...
myHeatmap<-function(x,t.colors=NA,fileName="cluster.cdt",linkage="complete",distance="pearson",contrast=2,returnSampleClust=F,returnGeneClust=F,rowNames=NA,rightMar=7,bottomMar=1,colNames=NA){
...
myHeatmap2<-function(x,t.colors=NA,fileName="cluster.cdt",linkage="complete",distance="pearson",contrast=2,returnSampleClust=F,rowNames=NA,rightMar=7,bottomMar=1,colNames=NA){
	...
	if(sum(is.na(t.colors))>0){
			heatmap(imageVals,Rowv=NA,Colv=as.dendrogram(sTree),
							col=cols(),labCol=colNames, scale="none",
							margins=c(bottomMar,rightMar),labRow=rowNames)
	}else{
		if(length(t.colors)>dim(imageVals)[2]){
			heatmap.plus(imageVals,Rowv=NA,Colv=as.dendrogram(sTree),
								col=cols(),labCol=colNames,labRow=rowNames,scale="none",
								ColSideColors=t.colors, margins=c(bottomMar,rightMar))
		}else{
			heatmap(imageVals,Rowv=NA,Colv=as.dendrogram(sTree),
...
myHeatmap3<-function(x,t.colors=NA,fileName="cluster.cdt",linkage="complete",distance="pearson",contrast=2,returnSampleClust=F,rowNames=NA,rightMar=7,bottomMar=1,colNames=NA){
	...
	if(sum(is.na(t.colors))>0){
			heatmap(imageVals,Rowv=as.dendrogram(gTree),Colv=NA,
							col=cols(),labCol=colNames, scale="none",
							margins=c(bottomMar,rightMar),labRow=rowNames)
	}else{
		if(length(t.colors)>dim(imageVals)[2]){
			heatmap.plus(imageVals,Rowv=as.dendrogram(gTree),Colv=NA,
								col=cols(),labCol=colNames,labRow=rowNames,scale="none",
								ColSideColors=t.colors, margins=c(bottomMar,rightMar))
		}else{
			heatmap(imageVals,Rowv=as.dendrogram(gTree),Colv=NA,
	...
	for( i in startPC:stopPC ){
		for(j in startPC:stopPC){
			...
			if(j == i){
				...
				strX<-paste("PC",i,sep=" ")
				strY<-paste("PC",j,sep=" ")
				plot(data.pca$x[,i],data.pca$x[,j], xlab=strX,pch=19,
...
overlapSets<-function(x,y){
	...
	y<-y[as.vector(as.character(rownames(x))),]
...
collapse<- function(x,allids,method){
        if(method=="mean"){
                out<- by(x,allids,mean,na.rm=T)
        }
        if(method=="median"){
                out<- by(x,allids,median,na.rm=T)
        }
        if(method=="sum"){
                out<- by(x,allids,sum,na.rm=T)
        }
        return(out)
 }

collapseIDs2<-function(x,allids=row.names(x),method="mean"){
        allids<-factor(allids)
        ids<- levels(allids)
        x.col<- NULL
        ...
        if(length(ids)==dim(x)[1]){ 
             ...
             return(x) 
	       	...
	       	x.col<- apply(x,2,collapse,allids,method)
				}
...
pcaEA<-function(x,classes,size=1,showLegend=T,legendloc="topright",mainStr="",startPC=1,stopPC=2,showNames=T,showClasses=F,axisExpansion=0,groupColors=NA){
	...
	xmin<-min(data.pca$x[,i])-abs(axisExpansion*min(data.pca$rotation[,i]))
	xmax<-max(data.pca$x[,i])+abs(axisExpansion*max(data.pca$rotation[,i]))
	ymin<-min(data.pca$x[,j])-abs(axisExpansion*min(data.pca$rotation[,j]))
	ymax<-max(data.pca$x[,j])+abs(axisExpansion*max(data.pca$rotation[,j]))
	plot(data.pca$x[,i],data.pca$x[,j], xlab=strX, ylab=strY, 
							main=strM, col=gr.labels,cex=size,pch="",xlim=c(xmin,xmax),ylim=c(ymin,ymax))
	if(showNames){
		text(data.pca$x[,i],data.pca$x[,j],labels=names(data.pca$rotation[,i]),cex=size*.6)
	}else{
		if(showClasses){
				text(data.pca$x[,i],data.pca$x[,j],labels=gr.labels.fac,cex=size*.6)
		}else{
				points(data.pca$x[,i],data.pca$x[,j],col=gr.labels,cex=size*1.5,pch=19)
		}
		if(showLegend){
